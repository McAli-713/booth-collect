...
一个基于 Node.js + PostgreSQL 的喷房作业数据采集系统，支持移动端表单提交、管理后台查看、Excel 导出和邮件通知。
...
4. 选择你刚才的 GitHub 仓库
5. 填写配置：
   - **Name**: `spray-booth-collector`（随便起）
...
6. 点击 **Advanced** → **Add Environment Variable**，添加以下变量：
   ...
   | `ADMIN_USERNAME` | 管理后台用户名，如 `admin` |
   | `ADMIN_PASSWORD` | 管理后台密码，自己设一个 |
   | `RESEND_API_KEY` | （可选）Resend API Key，见第三步 |
   | `NOTIFICATION_EMAIL` | （可选）接收通知的邮箱 |

7. 点击 **Create Web Service**，等待部署完成（约 2-3 分钟）
8. 部署成功后，Render 会给你一个域名，如 `https://spray-booth-collector.onrender.com`
...
### 表单提交页
- 访问：`https://你的域名.onrender.com/`
- 操作员在手机上填写喷房参数并提交
- 表单字段：喷房编号、操作员、产品、涂料、颜色、电压、电流、流量、压力、膜厚、质量结果等

### 管理后台
- 访问：`https://你的域名.onrender.com/admin`
- 输入你设置的 `ADMIN_USERNAME` / `ADMIN_PASSWORD` 登录
- 功能：查看所有记录、删除记录、导出 Excel

### Excel 导出
- 登录管理后台后，点击右上角「导出 Excel」
- 导出所有记录，包含全部字段
...
**Q: 表单字段能改吗？**
A: 可以。修改 `public/index.html` 的表单字段，同时修改 `db.js` 的表结构和 `server.js` 的插入语句。
